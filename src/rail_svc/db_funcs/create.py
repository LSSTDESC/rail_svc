...
async def create_rows_batched(
    ...
    *,
    ...
    logger.info("All batches completed", table=the_class.__name__, total_created=len(all_rows))
    return all_rows
...
async def bulk_insert_rows(
    ...
    *,
    ...
    logger.debug("Bulk inserting rows", table=the_class.__name__, count=len(rows_data))
    ...
    try:
        ...
        logger.info("Bulk insert completed", table=the_class.__name__, count=len(rows_data))
