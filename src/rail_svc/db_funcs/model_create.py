...
>>> from myapp.operations.model import model_creator
>>>
>>> async with session.begin():
...     model = await model_creator.create_row(
...         session,
...         name="RF_Model_v1",
...
Create model with IDs (no validation):

>>> async with session.begin():
...     model = await model_creator.create_row(
...         session,
...         name="Custom_Model",
...         path="models/custom.pkl",
...         algo_id=123,
...         catalog_tag_id=456,
...         validate_file=False
...     )
...
...         session,
...         name="Hybrid_Model",
...         path="models/hybrid.pkl",
...         algo_name="NeuralNetEstimator",
...         catalog_tag_id=789,
...         validate_file=True
...     )
"""
...
from ..db import Algorithm, CatalogTag, Model
from .row_create import RowCreateBase, RowCreateContext

logger = logging.getLogger(__name__)

__all__ = ["ModelCreate", "model_creator"]
...
class ModelCreate(RowCreateBase[Model]):
    ...
    async def get_create_kwargs(
        ...
        **extra_kwargs
        ...
        algo_id, algo_obj = await db_funcs.read.lookup_by_id_or_name(
            Algorithm, session, algo_id, algo_name, need_object=validate_file
        )

        # Resolve catalog_tag foreign key
        catalog_tag_id, catalog_tag_obj = await db_funcs.read.lookup_by_id_or_name(
            CatalogTag, session, catalog_tag_id, catalog_tag_name, need_object=validate_file
        )
...
model_creator: ModelCreate = ModelCreate(RowCreateContext.from_db_class(Model))
