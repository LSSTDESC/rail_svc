...
class ClientConfiguration(BaseSettings):
    ...
    service_url: str = Field(
        default="http://localhost:8080/rail-svc/v1",
        validation_alias="RAILL_SVC",
    )

    auth_token: str | None = Field(
        default=None,
        validation_alias="RAIL_TOKEN",
    )
    ...
    cookies: list[ClientCookie] | None = Field(
        ...
        default=None,
        validation_alias="RAIL_COOKIES",
    )

    timeout: float | None = Field(
        default=None,
        validation_alias="RAIL_TIMEOUT",
    )
