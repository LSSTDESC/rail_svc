...
class TestFilterRows:
    ...
    async def test_filter_rows_no_filters(self, session, sample_users):
        ...
        results = await filter_rows(DbTestUser, session, filters=None)
        assert len(results) == 5

    @pytest.mark.asyncio
    async def test_filter_rows_equality(self, session, sample_users):
        """Test filtering with equality operator."""
        results = await filter_rows(DbTestUser, session, filters=[Filter("status", FilterOp.EQ, "active")])
    ...
    async def test_filter_rows_not_equal(self, session, sample_users):
        ...
        results = await filter_rows(DbTestUser, session, filters=[Filter("role", FilterOp.NE, "user")])
    ...
    async def test_filter_rows_greater_than(self, session, sample_users):
        ...
        results = await filter_rows(DbTestUser, session, filters=[Filter("age", FilterOp.GT, 25)])
    ...
    async def test_filter_rows_greater_equal(self, session, sample_users):
        ...
        results = await filter_rows(DbTestUser, session, filters=[Filter("age", FilterOp.GE, 25)])
        assert len(results) == 4
        assert all(u.age >= 25 for u in results)
    ...
    async def test_filter_rows_less_than(self, session, sample_users):
        ...
        results = await filter_rows(DbTestUser, session, filters=[Filter("age", FilterOp.LT, 30)])
        assert len(results) == 3
    ...
    async def test_filter_rows_less_equal(self, session, sample_users):
        ...
        results = await filter_rows(DbTestUser, session, filters=[Filter("age", FilterOp.LE, 30)])
        assert len(results) == 4
        assert all(u.age <= 30 for u in results)
    ...
    async def test_filter_rows_in_operator(self, session, sample_users):
        ...
        results = await filter_rows(DbTestUser, session, filters=[Filter("id", FilterOp.IN, [1, 2, 3])])
        assert len(results) == 3
    ...
    async def test_filter_rows_not_in_operator(self, session, sample_users):
        ...
        results = await filter_rows(DbTestUser, session, filters=[Filter("id", FilterOp.NOT_IN, [1, 2])])
        assert len(results) == 3
    ...
    async def test_filter_rows_like_operator(self, session, sample_users):
        ...
        results = await filter_rows(DbTestUser, session, filters=[Filter("username", FilterOp.LIKE, "a%")])
    ...
    async def test_filter_rows_starts_with(self, session, sample_users):
        ...
        results = await filter_rows(
            DbTestUser, session, filters=[Filter("username", FilterOp.STARTS_WITH, "c")]
        )
        assert len(results) == 1
    ...
    async def test_filter_rows_is_null(self, session, sample_users):
        ...
        results = await filter_rows(DbTestUser, session, filters=[Filter("deleted_at", FilterOp.IS_NULL)])
        assert len(results) == 4
    ...
    async def test_filter_rows_is_not_null(self, session, sample_users):
        ...
        results = await filter_rows(DbTestUser, session, filters=[Filter("deleted_at", FilterOp.IS_NOT_NULL)])
        assert len(results) == 1
    ...
    async def test_filter_rows_between(self, session, sample_users):
        ...
        results = await filter_rows(DbTestUser, session, filters=[Filter("age", FilterOp.BETWEEN, [25, 30])])
        assert len(results) == 3
    ...
    async def test_filter_rows_in_operator_wrong_type(self, session, sample_users):
        ...
        with pytest.raises(ValueError, match="IN operator requires"):
            await filter_rows(DbTestUser, session, filters=[Filter("id", FilterOp.IN, "not_a_list")])
...
class TestFilterRowsStreaming:
    ...
    async def test_filter_rows_streaming_basic(self, session, sample_users):
        ...
        async for user in filter_rows_streaming(
            DbTestUser, session, filters=[Filter("status", FilterOp.EQ, "active")]
        ):
            results.append(user)
    ...
    async def test_filter_rows_streaming_with_limit(self, session, sample_users):
        ...
        results = []
        async for user in filter_rows_streaming(DbTestUser, session, filters=None, limit=3):
            results.append(user)
    ...
    async def test_filter_rows_streaming_ordered(self, session, sample_users):
        ...
        results = []
        async for user in filter_rows_streaming(
            DbTestUser, session, filters=None, order_by=OrderBy("age", descending=False)
        ):
            results.append(user)
...
class TestCountFilteredRows:
    ...
    async def test_count_all_rows(self, session, sample_users):
        ...
        count = await count_filtered_rows(DbTestUser, session, filters=None)
        assert count == 5
    ...
    async def test_count_no_matches(self, session, sample_users):
        ...
        count = await count_filtered_rows(
            DbTestUser, session, filters=[Filter("status", FilterOp.EQ, "nonexistent")]
        )
        assert count == 0
...
class TestFilterOne:
    ...
    async def test_filter_one_success(self, session, sample_users):
        ...
        user = await filter_one(DbTestUser, session, filters=[Filter("username", FilterOp.EQ, "alice")])
    ...
    async def test_filter_one_multiple_matches(self, session, sample_users):
        ...
        with pytest.raises(KeyError, match="Multiple .* rows found"):
            await filter_one(DbTestUser, session, filters=[Filter("status", FilterOp.EQ, "active")])
...
class TestFilterOneOrNone:
    ...
    async def test_filter_one_or_none_success(self, session, sample_users):
        ...
        user = await filter_one_or_none(
            DbTestUser, session, filters=[Filter("username", FilterOp.EQ, "alice")]
        )
    ...
    async def test_filter_one_or_none_multiple_matches(self, session, sample_users):
        ...
        with pytest.raises(KeyError, match="Multiple .* rows found"):
            await filter_one_or_none(DbTestUser, session, filters=[Filter("status", FilterOp.EQ, "active")])
...
class TestFindBy:
    ...
    async def test_find_by_single_field(self, session, sample_users):
        ...
        results = await find_by(DbTestUser, session, status="active")
        assert len(results) == 4
        assert all(u.status == "active" for u in results)
    ...
    async def test_find_by_multiple_fields(self, session, sample_users):
        ...
        results = await find_by(DbTestUser, session, status="active", role="user")
        assert len(results) == 2
    ...
    async def test_find_by_with_pagination(self, session, sample_users):
        ...
        results = await find_by(DbTestUser, session, status="active", skip=1, limit=2)
        assert len(results) == 2

    @pytest.mark.asyncio
    async def test_find_by_no_matches(self, session, sample_users):
        """Test find_by with no matches returns empty list."""
        results = await find_by(DbTestUser, session, status="nonexistent")
...
class TestFindOneBy:
    ...
    async def test_find_one_by_success(self, session, sample_users):
        ...
        user = await find_one_by(DbTestUser, session, username="alice")
        assert user.username == "alice"
        assert user.id == 1
    ...
    async def test_find_one_by_multiple_fields(self, session, sample_users):
        ...
        user = await find_one_by(DbTestUser, session, username="bob", role="user")
    ...
    async def test_find_one_by_not_found(self, session, sample_users):
        ...
        with pytest.raises(KeyError, match="No .* found"):
            await find_one_by(DbTestUser, session, username="nonexistent")
    ...
    async def test_find_one_by_multiple_matches(self, session, sample_users):
        ...
        with pytest.raises(KeyError, match="Multiple .* rows found"):
            await find_one_by(DbTestUser, session, status="active")
...
class TestEdgeCases:
    ...
    async def test_ordering_invalid_field(self, session, sample_users):
        ...
        with pytest.raises(AttributeError, match="does not have field"):
            await filter_rows(DbTestUser, session, filters=None, order_by=OrderBy("nonexistent"))
    ...
    async def test_large_in_list(self, session, sample_users):
        ...
        results = await filter_rows(DbTestUser, session, filters=[Filter("id", FilterOp.IN, large_list)])
    ...
    async def test_empty_in_list(self, session, sample_users):
        ...
        results = await filter_rows(DbTestUser, session, filters=[Filter("id", FilterOp.IN, [])])
        assert len(results) == 0
...
class TestFilterIntegration:
    ...
    async def test_pagination_consistency(self, session, sample_users):
        ...
        all_results = await filter_rows(
            DbTestUser, session, filters=None, order_by=OrderBy("id", descending=False)
        )
    ...
    async def test_count_matches_results(self, session, sample_users):
        ...
        count = await count_filtered_rows(DbTestUser, session, filters=filters)

        results = await filter_rows(DbTestUser, session, filters=filters)
    ...
    async def test_streaming_matches_regular(self, session, sample_users):
        ...
        # Regular query
        regular_results = await filter_rows(DbTestUser, session, filters=filters, order_by=order)

        # Streaming query
        streaming_results = []
        async for user in filter_rows_streaming(DbTestUser, session, filters=filters, order_by=order):
    ...
    async def test_find_by_matches_filter_rows(self, session, sample_users):
        ...
        # Using find_by
        find_by_results = await find_by(DbTestUser, session, status="active", role="user")
...
class TestFilterPerformance:
    ...
    async def test_complex_or_logic(self, session, sample_users):
        ...
        results = await filter_rows(DbTestUser, session, filters=filters, logical_op="or")
...
class TestCaseInsensitiveOperations:
    ...
    async def test_ilike_operator(self, session, sample_users):
        ...
        results = await filter_rows(
            DbTestUser, session, filters=[Filter("username", FilterOp.ILIKE, "ALICE")]
        )
...
@pytest.mark.asyncio
async def test_comparison_operators_parametrized(session, sample_users, op, value, expected_count):
    """Parametrized test for all comparison operators."""
    field = "status" if isinstance(value, str) else "age"

    results = await filter_rows(DbTestUser, session, filters=[Filter(field, op, value)])
...
async def empty_session(engine) -> AsyncIterator[AsyncSession]:
    ...
    async with async_session() as session:
        ...
        await session.execute(DbTestUser.__table__.delete())
