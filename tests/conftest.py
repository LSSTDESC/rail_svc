...
async def multiple_algorithms(session):
    return await create_all(session, [
...
async def sample_band(session):
    return await create(
        session, Band(name="g_band", band_wavelengths=[400.0, 500.0, 600.0], band_transmission=[0.1, 0.9, 0.2])
    )
...
async def sample_catalog_band_assoc(session, sample_catalog_tag, sample_band):
    return await create(session, CatalogBandAssoc(
...
async def sample_dataset(session, sample_catalog_tag):
    return await create(session, Dataset(
        name="test_dataset",
        n_objects=10000,
...
async def multiple_datasets(session, sample_catalog_tag):
    return await create_all(session, [
        Dataset(name="photometric_data", n_objects=50000, path="/data/photometric.hdf5",
                is_collection=False, catalog_tag_id=sample_catalog_tag.id_),
        Dataset(name="spectroscopic_data", n_objects=5000, path="/data/spectroscopic.hdf5",
                is_collection=False, catalog_tag_id=sample_catalog_tag.id_),
        Dataset(name="combined_collection", n_objects=55000, path="/data/combined/",
                is_collection=True, catalog_tag_id=sample_catalog_tag.id_),
    ])
...
async def matched_dataset(session, sample_catalog_tag):
    return await create(session, Dataset(
        name="matched_catalog", n_objects=10000, path="/data/matched.hdf5",
        is_collection=False, catalog_tag_id=sample_catalog_tag.id_,
    ))
...
async def component_dataset_1(session, sample_catalog_tag):
    return await create(session, Dataset(
        name="gaia_dr3", n_objects=5000, path="/data/gaia.hdf5",
        is_collection=False, catalog_tag_id=sample_catalog_tag.id_,
    ))
...
async def component_dataset_2(session, sample_catalog_tag):
    return await create(session, Dataset(
        name="sdss_dr17", n_objects=6000, path="/data/sdss.hdf5",
        is_collection=False, catalog_tag_id=sample_catalog_tag.id_,
    ))
...
async def sample_dataset_assoc(session, matched_dataset, component_dataset_1):
    return await create(session, DatasetAssoc(
...
async def multiple_dataset_assocs(session, matched_dataset, component_dataset_1, component_dataset_2):
    return await create_all(session, [
        DatasetAssoc(name="gaia_to_match", matched_dataset_id=matched_dataset.id_,
                     component_dataset_id=component_dataset_1.id_),
        DatasetAssoc(name="sdss_to_match", matched_dataset_id=matched_dataset.id_,
...
async def sample_estimates(session, sample_dataset, sample_estimator):
    return await create(session, Estimates(
        name="test_estimates", n_objects=10000, path="/results/test_estimates.hdf5",
        dataset_id=sample_dataset.id_, estimator_id=sample_estimator.id_,
    ))
...
async def multiple_estimates(session, sample_dataset, sample_estimator):
    return await create_all(session, [
        Estimates(name=f"estimates_v{i}", n_objects=5000, path=f"/results/estimates_v{i}.hdf5",
                  dataset_id=sample_dataset.id_, estimator_id=sample_estimator.id_)
        for i in range(1, 4)
    ])
...
async def sample_estimator(session, sample_model):
    return await create(session, Estimator(
        name="knn_estimator", config={"n_neighbors": 5, "weights": "distance"},
...
async def sample_model(session, sample_algorithm, sample_catalog_tag):
    return await create(session, Model(
        name="knn_model", path="/models/knn.pkl",
        algo_id=sample_algorithm.id_, catalog_tag_id=sample_catalog_tag.id_,
    ))
...
async def multiple_models(session, sample_algorithm, sample_catalog_tag):
    return await create_all(session, [
        Model(name=f"model_v{i}", path=f"/models/model_v{i}.pkl",
              algo_id=sample_algorithm.id_, catalog_tag_id=sample_catalog_tag.id_)
        for i in range(1, 4)
    ])
...
async def sample_sed(session):
    return await create(session, Sed(
        name="elliptical_01",
...
async def multiple_seds(session):
    return await create_all(session, [
        Sed(name="spiral_01", sed_wavelengths=[300.0, 400.0, 500.0, 600.0], sed_values=[0.2, 0.5, 0.8, 0.6]),
        Sed(name="starburst_01", sed_wavelengths=[300.0, 400.0, 500.0, 600.0], sed_values=[0.8, 0.6, 0.3, 0.1]),
        Sed(name="quiescent_01", sed_wavelengths=[300.0, 400.0, 500.0, 600.0], sed_values=[0.05, 0.1, 0.3, 0.5]),
    ])
...
async def sample_filter_ab(session, sample_band, sample_sed):
    return await create(session, FilterAB(
        name="g_band_elliptical_01",
        band_id=sample_band.id_, sed_id=sample_sed.id_,
...
async def multiple_filter_abs(session, sample_band, multiple_seds):
    return await create_all(session, [
        FilterAB(name=f"g_band_{multiple_seds[i].name}", band_id=sample_band.id_,
                 sed_id=multiple_seds[i].id_, redshifts=[0.0, 0.5, 1.0], fluxes=fluxes)
